...
趁着篇幅还短，这里再介绍一下另一个 CPAN::Inject 的应用 M<cpanf>

如果您跟我一样订阅了 CPAN 的 A<http://search.cpan.org/uploads.rdf|RSS) 的话，可能看见一个新模块或者模块的新版本时，会忍不住想试试。但是如果这个模块还没有到到您的镜像时，您无法使用 cpan 来安装（即使 reload index 也没用）。这个时候，您可以尝试下用 cpanf 来安装这个模块。
