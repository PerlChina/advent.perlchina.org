...
但你想安装一个模块，却没有 root 权限的时候（在某个虚拟主机下）或者你不想覆盖该模块时，local::lib 应该是您所能想到的第一个模块。

以 dreamhost 主机为例，

步骤一，找到下载链接。打开 A<http://search.cpan.org/dist/local-lib/> 得到下载链接 C<http://search.cpan.org/CPAN/authors/id/A/AP/APEIRON/local-lib-1.004009.tar.gz>
...
你可能想登录时就直接添加 local::lib 目录，添加下面一行到 ~/.bash_profile
